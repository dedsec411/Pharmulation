...
- [ ] **Fix the admin panel's silent no-op writes** (`admin.tsx`): "Promote user" and "Delete case" currently fail silently — no RLS policy/grant permits them, and the result isn't even checked. Route both through a `createServerFn` using the already-defined-but-unused `supabaseAdmin` (service-role client), with a server-side `is_admin` check.
