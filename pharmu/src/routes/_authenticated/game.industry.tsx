  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        ...
        >
          <PackageCheck className={`size-7 ${released ? "text-amber-700 dark:text-amber-300" : "text-destructive"}`} />
        </motion.div>
        <div className="min-w-0">
          <p className={`text-[11px] font-black uppercase tracking-[0.18em] ${
            released ? "text-amber-700 dark:text-amber-300" : "text-destructive"
          }`}>
            ...
            {ingredients.map((i: any) => (
              <li key={i.name} className="grid grid-cols-[18px_1fr] gap-2 border-b border-amber-200/10 pb-1.5 text-xs last:border-b-0">
                <span className="mt-0.5 grid h-3.5 w-3.5 place-items-center border border-amber-200/35 bg-amber-400/10">
                  <Check className="size-2.5 text-amber-700 dark:text-amber-300" />
                </span>
