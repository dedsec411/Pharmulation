          ...
          >
            ...
            >
              ...
              <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
                <p className="mb-3 text-xs text-slate-600 dark:text-slate-400">{section.caption}</p>
                <dl className="space-y-2">
                  {section.entries.map((entry) => (
                    <div
                      key={entry.term}
                      className="rounded-xl border border-slate-300 dark:border-slate-700/50 bg-slate-900/[0.06] dark:bg-slate-900/50 px-4 py-3"
                    >
                      <div className="flex flex-wrap items-baseline justify-between gap-2">
                        <dt className="font-bold text-slate-900 dark:text-slate-100">{entry.term}</dt>
                        {/* On the formula page this is the number to weigh; on the
                            others it is the sentence that explains the step. */}
                        <dd className="font-mono text-sm text-amber-700 dark:text-amber-200">{entry.detail}</dd>
                      </div>
                      {entry.note && <p className="mt-1.5 text-xs leading-relaxed text-slate-600 dark:text-slate-400">{entry.note}</p>}
                    </div>
                  ))}
                </dl>
              </div>

              <p className="border-t border-amber-200/15 px-5 py-2.5 text-[11px] text-slate-600 dark:text-slate-500">
