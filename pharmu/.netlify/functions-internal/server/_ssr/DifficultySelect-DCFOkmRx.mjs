...
import { u as useAuthStore, a as useActiveCaseStore, B as Button } from "./router-Cn57AZkw.mjs";
...
import { T as Trophy, a as CircleCheck, Z as CircleX, _ as CircleAlert, $ as RotateCw, a0 as House, R as ArrowLeft, A as Activity, a1 as Zap, a2 as Play, a3 as Pause, y as Lightbulb, O as TriangleAlert, a4 as Gauge, a5 as ShieldAlert, v as Sparkles, a6 as MessageCircle } from "../_libs/lucide-react.mjs";
...
function useCaseLoader(mode, difficulty) {
  ...
  const load = reactExports.useCallback(async () => {
    ...
    setLoading(true);
    const c = mode === "otc" ? await fetchOtcPilotCase(reloadKey) : await fetchTemplateCase(mode, difficulty, profile?.user_id, profile?.level) ?? await fetchRandomCase(mode, difficulty);
    setCaseData(c);
    setActiveCase(c);
    setLoading(false);
...
async function fetchOtcPilotCase(reloadKey) {
  const targetIndex = reloadKey % OTC_PILOT_CASE_IDS.length;
  const targetId = OTC_PILOT_CASE_IDS[targetIndex];
  const { data, error } = await supabase.from("cases").select("*").eq("id", targetId).maybeSingle();
...
function buildOtcPilotFallbackCase(index, idOverride) {
